...
$$
\sum_j \mathbf{K}[i,j] = \sum_j \text{T}[i,j] - \sum_j F[i,j] \cdot P[j]
$$
...
$$
[(\mathbf{I} - \mathbf{K})\mathbf{1}]_i = 1 - \left(1 - \sum_j F[i,j] \cdot (1 - b_j)\right)
$$

$$
[(\mathbf{I} - \mathbf{K})\mathbf{1}]_i = \sum_j F[i,j] \cdot (1 - b_j)
$$
...
The left side equals $\mathbf{A} + \mathbf{R}$ (from [Theorem @thm-spectral_radius_unity]):
