...
For the anisotropic radiative transfer formulation with anisotropy matrix $\mathbf{\Gamma}$ satisfying $\mathbf{\Gamma}_{ij} \in [0, 1]$, the interaction-reflection-scattering matrix $\mathbf{K}$ constructed via row-stochastic normalization is guaranteed to be non-negative: $K_{ij} \geq 0$ for all $i, j$.
...
$$
\mathbf{K}[i,j] = \frac{\mathbf{K}_{\text{init}}[i,j]}{S[i]} - \mathbf{F}[i,j] \cdot P[j] \left(1 - \frac{1}{S[i]}\right)
$$

Substituting $\mathbf{K}_{\text{init}}[i,j] = \mathbf{F}[i,j] \cdot \Gamma[i,j] \cdot (1-P[j])$:

$$
\mathbf{K}[i,j] = \frac{\mathbf{F}[i,j] \cdot \Gamma[i,j] \cdot (1-P[j])}{S[i]} - \mathbf{F}[i,j] \cdot P[j] \left(1 - \frac{1}{S[i]}\right)
$$
...
$$
\mathbf{F}[i,j] \cdot \Gamma[i,j] \cdot (1-P[j]) \geq \mathbf{F}[i,j] \cdot P[j] \cdot (S[i] - 1)
$$
...
$$
S[i] - 1 = \sum_k \mathbf{F}[i,k] \cdot (\Gamma[i,k] - 1) \cdot (1-P[k])
$$
...
Since $\mathbf{F}[i,k] \geq 0$ and $(1-P[k]) \geq 0$:

$$
\mathbf{F}[i,k] \cdot (\Gamma[i,k] - 1) \cdot (1-P[k]) \leq 0 \quad \forall k
$$

Therefore:

$$
S[i] - 1 = \sum_k \mathbf{F}[i,k] \cdot (\Gamma[i,k] - 1) \cdot (1-P[k]) \leq 0
$$
...
$$
\mathbf{F}[i,j] \cdot P[j] \cdot (S[i] - 1) \leq 0
$$

And since $\mathbf{F}[i,j] \cdot \Gamma[i,j] \cdot (1-P[j]) \geq 0$ (all factors are non-negative), we have:

$$
\mathbf{F}[i,j] \cdot \Gamma[i,j] \cdot (1-P[j]) \geq 0 \geq \mathbf{F}[i,j] \cdot P[j] \cdot (S[i] - 1)
$$
...
The non-negativity proof reveals a fundamental constraint: the upper bound $\mathbf{Γ} \leq 1$ ensures $S[i] \leq 1$, which makes the normalization correction term non-positive, thereby guaranteeing $\mathbf{K}_{ij} \geq 0$.

**Importantly, no lower bound on Γ is required for non-negativity.** The full range $\mathbf{Γ} \in [0, 1]$ is permissible, providing maximum flexibility for representing anisotropic reflection-scattering distributions while maintaining physical validity.
...
$$
\sum_j \mathbf{K}[i,j] = \sum_j \text{T}[i,j] - \sum_j \mathbf{F}[i,j] \cdot P[j]
$$

$$
\sum_j \mathbf{K}[i,j] = 1 - \sum_j \mathbf{F}[i,j] \cdot P[j]
$$
...
$$
\sum_j \mathbf{K}[i,j] = 1 - \sum_j \mathbf{F}[i,j] \cdot (1 - b_j)
$$

**Step 3:** Compute $[(\mathbf{I} - \mathbf{K})\mathbf{1}]_i$.

$$
[(\mathbf{I} - \mathbf{K})\mathbf{1}]_i = 1 - \sum_j \mathbf{K}[i,j]
$$

Substituting from Step 2:

$$
[(\mathbf{I} - \mathbf{K})\mathbf{1}]_i = 1 - \left(1 - \sum_j \mathbf{F}[i,j] \cdot (1 - b_j)\right)
$$

$$
[(\mathbf{I} - \mathbf{K})\mathbf{1}]_i = \sum_j \mathbf{F}[i,j] \cdot (1 - b_j)
$$
