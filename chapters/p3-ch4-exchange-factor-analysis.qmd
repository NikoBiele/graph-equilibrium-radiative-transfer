...
$$
(\mathbf{1}-\mathbf{b})^T \mathbf{M} = (\mathbf{1}-\mathbf{b})^T \mathbf{C} + (\mathbf{1}-\mathbf{b})^T (\mathbf{e}_j (\mathbf{A}^T)_j) = (1-b_j)(\mathbf{A}^T)_j
$$

Since $\mathbf{A} \geq \mathbf{0}$, $(1-b_j) > 0$, and step 2 ensures $\mathbf{A}$ has at least one positive entry in each column, it is obtained that $(\mathbf{1}-\mathbf{b})^T \mathbf{M} \neq \mathbf{0}$.
