...
The exchange factor transformation described in Chapter @sec-exchange_fact_trans enables analytical treatment of multiple reflection-scattering processes in radiative transfer by transforming the exchange factor matrix $\textbf{F}$ into absorption and reflection-scattering matrices $\textbf{A}$ and $\textbf{R}$. The method assumes isotropic reflection-scattering behavior, where the probability of reflection-scattering from element $j$ is independent of the incident element $i$. This is represented by a column-constant single reflection-scattering matrix $\textbf{B}$.
...
The anisotropic extension introduces a full matrix $\textbf{\Gamma}$ (Gamma) to characterize directional dependence of reflection-scattering:

$$
\textbf{\Gamma} \in \mathbb{R}^{N \times N}, \quad 0 \leq \Gamma_{ij} \leq 1 \quad \forall i,j
$$ {#eq-gamma-bounds}
...
### Relationship to Isotropic Case

When $\textbf{\Gamma} = \textbf{1}$ (all ones), the anisotropic formulation reduces exactly to the isotropic case, ensuring backward compatibility and providing a natural basis for proof by continuity arguments.

## Construction of the Anisotropic Interaction Matrix

The anisotropic interaction-reflection-scattering matrix $\textbf{K}$ is constructed through a multi-step process that ensures physical consistency.
...
$$
B_{\text{init}}[i,j] = \Gamma[i,j] \cdot (1 - P[j])
$$ {#eq-b-init}

where $\textbf{P}$ is the diagonal emissivity matrix with entries $P[j] = 1 - b_j$, and $b_j$ is the reflection-scattering coefficient of element $j$.

Each column $j$ is scaled by the reflection-scattering probability $(1-P[j]) = b_j$ of the receiving element.
...
$$
K_{\text{init}}[i,j] = F[i,j] \cdot B_{\text{init}}[i,j] = F[i,j] \cdot \Gamma[i,j] \cdot (1 - P[j])
$$ {#eq-k-init}
...
$$
\text{enforce\_unity}[i,j] = K_{\text{init}}[i,j] + F[i,j] \cdot P[j]
$$ {#eq-enforce-unity}

Each element either reflects-scatters ($\textbf{K}_{\text{init}}$) or is absorbed ($\textbf{F} \cdot \textbf{P}$).
...
$$
S[i] = \sum_j \text{enforce\_unity}[i,j] = \sum_j (K_{\text{init}}[i,j] + F[i,j] \cdot P[j])
$$ {#eq-row-sums}
...
$$
\text{rowsums\_unity}[i,j] = \frac{\text{enforce\_unity}[i,j]}{S[i]}
$$ {#eq-normalize}
...
$$
K[i,j] = \text{rowsums\_unity}[i,j] - F[i,j] \cdot P[j]
$$ {#eq-k-final}
...
Since $\textbf{F}$ is row-stochastic ($\sum_j F[i,j] = 1$), we can write:
...
When $\textbf{\Gamma} = \textbf{1}$ (all ones), we have:
...
This recovers the isotropic case exactly: $\textbf{K} = \textbf{F} \circ \textbf{B}$, where $\textbf{B}$ is the column-constant matrix of reflection-scattering coefficients.

## Subsequent Transformation

Once $\textbf{K}$ is constructed, the remainder of the exchange factor transformation proceeds identically to the isotropic case:

### Steady-State Path Matrix

$$
\textbf{S}_\infty = (\textbf{I} - \textbf{K})^{-1} \textbf{F}
$$ {#eq-s-infinity}

### Absorption and Reflection-Scattering Matrices

$$
\textbf{A} = \textbf{P}(\textbf{I}-\textbf{K})^{-1}(\textbf{F}-\textbf{K})
$$ {#eq-a-matrix}

$$
\textbf{R} = \textbf{P}(\textbf{I}-\textbf{K})^{-1}\textbf{K}
$$ {#eq-r-matrix}

Or equivalently, using the Hadamard product notation:

$$
\textbf{A} = (\textbf{1} - \textbf{B})^T \circ \textbf{S}_\infty \circ (\textbf{1} - \textbf{B})
$$ {#eq-a-hadamard}

$$
\textbf{R} = (\textbf{1} - \textbf{B})^T \circ \textbf{S}_\infty \circ \textbf{B}
$$ {#eq-r-hadamard}

Note: In the anisotropic case, $\textbf{B}$ is no longer column-constant, but the definitions remain valid.

### System Matrices

$$
\textbf{C} = \textbf{I} - \textbf{A}^T - \textbf{R}^T
$$ {#eq-c-matrix}

$$
\textbf{D} = \textbf{I} - \textbf{R}^T
$$ {#eq-d-matrix}
...
$$
\textbf{M}\textbf{j} = \textbf{h}
$$ {#eq-system}

where $\textbf{M}$ is assembled row-by-row from $\textbf{C}$ (for known sources) and $\textbf{D}$ (for known emissive powers), and $\textbf{j}$ is the total radiant power vector.
...
3. **Backward Compatibility**: Reduces exactly to the isotropic case when $\textbf{\Gamma} = \textbf{1}$, ensuring consistency with established methods.
...
### Specular-Diffuse Surfaces, Anisotropic Scattering in Participating Media and Surface Microstructure
