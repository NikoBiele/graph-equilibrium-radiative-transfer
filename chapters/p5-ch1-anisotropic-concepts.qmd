...
where $\mathbf{P}$ is the diagonal emissivity matrix with entries $P[j] = 1 - b_j$, and $b_j$ is the reflection-scattering coefficient of element $j$.
...
Note: In the anisotropic case, $\mathbf{B}$ is no longer column-constant, but the definitions remain valid.
