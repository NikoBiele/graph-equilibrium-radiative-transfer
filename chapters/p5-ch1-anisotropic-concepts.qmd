...
$$
\mathbf{B}_{\text{init}}_{ij} = \mathbf{\Gamma}_{ij} \cdot (1 - \mathbf{P}_{jj})
$$ {#eq-b-init}

where $\mathbf{P}$ is the diagonal emissivity matrix with entries $\mathbf{P}_{jj} = 1 - \mathbf{b}_j$, and $\mathbf{b}_j$ is the reflection-scattering coefficient of element $j$.

Each column $j$ is scaled by the reflection-scattering probability $(1-\mathbf{P}_{jj}) = \mathbf{b}_j$ of the receiving element.
...
$$
\mathbf{K}_{\text{init}}_{ij} = \mathbf{F}_{ij} \cdot \mathbf{B}_{\text{init}}_{ij} = \mathbf{F}_{ij} \cdot \mathbf{\Gamma}_{ij} \cdot (1 - \mathbf{P}_{jj})
$$ {#eq-k-init}
...
$$
\mathbf{U}_{ij} = \mathbf{K}_{\text{init}}_{ij} + \mathbf{F}_{ij} \cdot \mathbf{P}_{jj}
$$ {#eq-enforce-unity}
...
$$
\mathbf{s}_i = \sum_j \text{U}_{ij} = \sum_j (K_{\text{init}}_{ij} + \mathbf{F}_{ij} \cdot \mathbf{P}_{jj})
$$ {#eq-row-sums}

Expanding:

$$
\mathbf{s}_i = \sum_j \mathbf{F}_{ij} \cdot (\Gamma_{ij} \cdot (1-\mathbf{P}_{jj}) + \mathbf{P}_{jj})
...
$$
\text{T}_{ij} = \frac{\mathbf{U}_{ij}}{\mathbf{s}_i}
$$ {#eq-normalize}
...
$$
K_{ij} = \text{T}_{ij} - \mathbf{F}_{ij} \cdot \mathbf{P}_{jj}
$$ {#eq-k-final}

Substituting from @eq-normalize:

$$
K_{ij} = \frac{K_{\text{init}}_{ij} + \mathbf{F}_{ij} \cdot \mathbf{P}_{jj}}{\mathbf{s}_i} - \mathbf{F}_{ij} \cdot \mathbf{P}_{jj}
$$ {#eq-k-explicit}

Simplifying:

$$
K_{ij} = \frac{K_{\text{init}}_{ij}}{\mathbf{s}_i} - \mathbf{F}_{ij} \cdot \mathbf{P}_{jj} \left(1 - \frac{1}{\mathbf{s}_i}\right)
$$ {#eq-k-simplified}
...
$$
\mathbf{s}_i - 1 = \sum_j \mathbf{F}_{ij} \cdot (\Gamma_{ij} \cdot (1-\mathbf{P}_{jj}) + \mathbf{P}_{jj} - 1)
$$ {#eq-s-minus-one}

Simplifying:

$$
\mathbf{s}_i - 1 = \sum_j \mathbf{F}_{ij} \cdot (\Gamma_{ij} - 1) \cdot (1-\mathbf{P}_{jj})
$$ {#eq-s-minus-one-simple}

Since $\Gamma_{ij} \leq 1$, $\mathbf{F}_{ij} \geq 0$, and $(1-\mathbf{P}_{jj}) \geq 0$:
...
$$
K_{ij} = \mathbf{F}_{ij} - \mathbf{F}_{ij} \cdot \mathbf{P}_{jj} = \mathbf{F}_{ij} \cdot (1-\mathbf{P}_{jj})
$$
